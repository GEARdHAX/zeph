...
// non-member or a soft-removed row — callers treat null as "404, not a
// distinguishable 403" to avoid leaking group existence to non-members.
const getMembership = (groupId, userId) => GroupMember.findOne({ group: groupId, user: userId, active: true });
...
const getMembershipWithFallback = async (groupId, userId) => {
  ...
  return isLegacyMember ? { role: Roles.MEMBER, user: userId, group: groupId } : null;
};
...
module.exports = {
  Roles, ROLE_RANK, Capabilities, ROLE_CAPABILITIES,
  getMembership, getMembershipWithFallback, hasCapability, canChangeRole, canRemoveMember, isPrivileged,
};
