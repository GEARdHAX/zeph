...
function User({ user, selected, onSelect }) {
  ...
  return (
    <div className="flex h-[54px] cursor-pointer items-center border-b hover:bg-accent" onClick={onSelect}>
      ...
      <div className="flex items-center pr-1">
        <div className={`flex h-full items-center p-1 ${selected ? 'text-blue-700' : 'text-muted-foreground'}`}>
          {selected ? <MinusCircle className="h-4 w-4" /> : <PlusCircle className="h-4 w-4" />}
        </div>
      </div>
    </div>
  );
